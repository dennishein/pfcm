...
def create_model(
    ...
    return UNetModel(
        ...
        out_channels=(1 if not learn_sigma else 2), #1,2 (3 if not learn_sigma else 6),
...
def create_ema_and_scales_fn(
    ...
    def ema_and_scales_fn(step):
        if target_ema_mode == "fixed" and scale_mode == "fixed":
            ...
            target_ema = 1.0
        elif target_ema_mode == "iCT" and scale_mode == "iCT":
            target_ema = 0.0
            K_prime = np.floor(total_steps / (np.log2(np.floor(end_scales / start_scales))+1.0)) #1
            scales = np.minimum(start_scales*2**np.floor(step / K_prime),end_scales)+1.0
        else:
