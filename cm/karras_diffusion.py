...
# added to solve issue when not running as root 
import os
os.environ['PIQ_HOME'] = './.cache'

def get_weightings(weight_schedule, snrs, sigma_data, t=None, next_t=None):
...
class KarrasDenoiser:
    ...
    def training_losses(self, model, x_start, sigmas, aug_dim=None, model_kwargs=None, noise=None):
        # added for supervised learning
        n_ch = x_start.size(1) // 2
        x_cond = x_start[:,0:n_ch,:,:].clone()
        x_start = x_start[:,n_ch:,:,:].clone()
        ...
        if aug_dim is not None:
          ### PFGM++ ###
          D = np.float32(aug_dim) 
          N = x_start.size(-1)*x_start.size(-1)*x_start.size(1)
          sigma = sigmas # effective resize since not using append_dims 
          r = sigma * np.sqrt(D).astype(np.float32)
          # sampling form inverse-beta distribution
          samples_norm = np.random.beta(a=N / 2., b=D / 2.,
                                        size=x_start.size(0)*n_ch)
          samples_norm = np.clip(samples_norm, 1e-3, 1-1e-3)
          inverse_beta = samples_norm / (1 - samples_norm + 1e-8)
          inverse_beta = th.from_numpy(inverse_beta).to(x_start.device)
          # sampling from p_r(R) by change-of-variable
          samples_norm = r * th.sqrt(inverse_beta + 1e-8)
          samples_norm = samples_norm.view(len(samples_norm), -1)
          # uniformly sample the angle direction
          gaussian = th.randn(x_start.size(0), N).to(samples_norm.device)
          unit_gaussian = gaussian / th.norm(gaussian, p=2, dim=1, keepdim=True)
          # construct the perturbation for x
          perturbation_x = unit_gaussian * samples_norm
          perturbation_x = perturbation_x.float()
          # get perturbed x 
          x_t = x_start + perturbation_x.view_as(x_start) 
        else:
          x_t = x_start + noise * append_dims(sigmas, dims)     
     
        # adjusted for supervised learning 
        model_output, denoised = self.denoise(model, th.cat((x_cond,x_t),1), sigmas, **model_kwargs)        
    ...
    def consistency_losses(
        ...
        def heun_solver(samples, t, next_t, x0):
            ...
            # added for supervised learning
            n_ch = x.size(1) // 2
            x_cond = x[:,0:n_ch,:,:].clone()
            x = x[:,n_ch:,:,:].clone()
            
            if teacher_model is None:
                denoiser = x0
            else:
                denoiser = teacher_denoise_fn(th.cat((x_cond,x),1), t)
            ...
            if teacher_model is None:
                ...
                denoiser = teacher_denoise_fn(th.cat((x_cond,samples),1), next_t)
        ...
        def euler_solver(samples, t, next_t, x0):
            ...
            if teacher_model is None:
                ...
                denoiser = teacher_denoise_fn(th.cat((x_cond,x),1), t)
            d = (x - denoiser) / append_dims(t, dims) # x_t-x_start = n
            samples = x + d * append_dims(next_t - t, dims)
        ...
        if aug_dim is not None:
          ...
          distiller = denoise_fn(th.cat((x_cond,x_t),1), t)

          if teacher_model is None:
            #x_t2 = euler_solver(th.cat((x_cond,x_t),1), t, t2, x_start).detach()
            sigma = t2 
            r = sigma * np.sqrt(D).astype(np.float32)
            # sampling from p_r(R) by change-of-variable
            samples_norm = r * th.sqrt(inverse_beta + 1e-8)
            ...
            perturbation_x = perturbation_x.float()
            # get perturbed x 
            x_t2 = x_start + perturbation_x.view_as(x_start) 
          else:
              x_t2 = heun_solver(th.cat((x_cond,x_t),1), t, t2, x_start).detach()
        else:
        ...
        distiller_target = target_denoise_fn(th.cat((x_cond,x_t2),1), t2)
        distiller_target = distiller_target.detach()

        snrs = self.get_snr(t)
        if self.weight_schedule == "iCT":
          weights = get_weightings(self.weight_schedule, snrs, self.sigma_data, t, t2)
        else:
    ...
    def progdist_losses(
        ...
        if self.loss_norm == "l1":
            ...
            c = 0.00054*np.sqrt(N)
            diffs = th.sqrt((distiller - distiller_target) ** 2 + c**2)
            loss = (mean_flat(diffs-c)) * weights
        elif self.loss_norm == "lpips":
            if x_start.shape[-1] < 256:
    ...
    # corresponds to teacher_diffusion.denoise 
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        import torch.distributed as dist

        # added for supervised learning
        n_ch = x_t.size(1) // 2
        x_cond = x_t[:,0:n_ch,:,:].clone()
        x_t = x_t[:,n_ch:,:,:].clone()
        ...
        model_output = model(c_in * th.cat((x_cond,x_t),1), rescaled_t, **model_kwargs)
...
def karras_sample(
    ...
    if x_cond is not None:
      if D is not None:
        ...
        for seed in range(shape[0]):
            beta_gen = Beta(th.FloatTensor([N / 2.]), th.FloatTensor([D / 2.]))
            sample_norm = beta_gen.sample().to(device)
            inverse_beta = sample_norm / (1-sample_norm)
            sample_norm = th.sqrt(inverse_beta) * sigma_max * np.sqrt(D)
            #sample_norm = th.sqrt(inverse_beta) * sigmas[32] * np.sqrt(D)
            gaussian = th.randn(N).to(sample_norm.device)
            unit_gaussian = gaussian / th.norm(gaussian, p=2)
            init_sample = unit_gaussian * sample_norm
            latent_list.append(init_sample.reshape((1, *shape[1:])))
        x_T = th.cat(latent_list, dim=0)
      else:
        x_T = generator.randn(*shape, device=device) * sigma_max
      x_c = x_cond.to(device)
      x_T = th.cat((x_c,x_T),1)
    else:
      x_T = generator.randn(*shape, device=device) * sigma_max
    
    sample_fn = {
        "heun": sample_heun,
        "heun_h": sample_heun_h,
        "dpm": sample_dpm,
        "ancestral": sample_euler_ancestral,
        "onestep": sample_onestep,
        "onestep_h": sample_onestep_h,
        "onestep_hijack": sample_onestep_hijack,
        "progdist": sample_progdist,
        "euler": sample_euler,
        "multistep": stochastic_iterative_sampler,
        "multistep_h": stochastic_iterative_sampler_h
    }[sampler]

    if sampler in ["heun", "heun_h","dpm"]:
        sampler_args = dict(
            s_churn=s_churn, s_tmin=s_tmin, s_tmax=s_tmax, s_noise=s_noise
        )
    elif sampler == "multistep" or sampler == "multistep_h":
        sampler_args = dict(
            ts=ts, t_min=sigma_min, t_max=sigma_max, rho=diffusion.rho, steps=steps
        )
    elif sampler == 'onestep_hijack':
        sampler_args = dict(
            hijack=hijack, weight=weight
        )
    else:
        sampler_args = {}
    ...
    return x_0.clamp(-1, 1) # => output from sample_fn shoud be [-1,1]
...
def sample_heun_h(
    ...
    for i in indices:
        ...
        if gamma > 0:
            x = x + eps * (sigma_hat**2 - sigmas[i] ** 2) ** 0.5
        #denoised = denoiser(x, sigma_hat * s_in)
        denoised = denoiser(th.cat((x_c,x),1), sigma_hat * s_in)
        d = to_d(x, sigma_hat, denoised)
        if callback is not None:
            callback(
                {
        ...
        if sigmas[i + 1] == 0:
            ...
            x_2 = x + d * dt
            #denoised_2 = denoiser(x_2, sigmas[i + 1] * s_in)
            denoised_2 = denoiser(th.cat((x_c,x_2),1), sigmas[i + 1] * s_in)
            d_2 = to_d(x_2, sigmas[i + 1], denoised_2)
            d_prime = (d + d_2) / 2
            x = x + d_prime * dt
    return x
...
def sample_heun_h_alt(
    ...
    for i in indices[len(indices)-1-hijack:]:
        gamma = (
            min(s_churn / (len(sigmas) - 1), 2**0.5 - 1)
            if s_tmin <= sigmas[i] <= s_tmax
            else 0.0
...
def sample_onestep_hijack(
    ...
    if hijack==1:
        s_in = x.new_ones([x.shape[0]])
        return distiller(x, sigmas[0] * s_in)*weight+(1-weight)*x[:,0:1,:,:] 
    else:
        x[:,1:,:,:] = x[:,0:1,:,:] # replace noise with condition
        s_in = x.new_ones([x.shape[0]])
        return distiller(x, sigmas[hijack] * s_in)*weight+(1-weight)*x[:,0:1,:,:] 
...
@th.no_grad()
def stochastic_iterative_sampler_h(
    distiller,
    x,
    sigmas,
    generator,
    ...
    for i in range(len(ts)-1): 
        t = (t_max_rho + ts[i] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        #x0 = distiller(x, t * s_in)
        #print('sending through cm')
        x0 = distiller(th.cat((x_c,x),1), t * s_in)
        #if i < len(ts)-1:
        next_t = (t_max_rho + ts[i + 1] / (steps - 1) * (t_min_rho - t_max_rho)) ** rho
        next_t = np.clip(next_t, t_min, t_max)
        #print('adding noise')
